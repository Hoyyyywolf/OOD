...
<img src="https://github.com/Hoyyyywolf/Images/raw/master/OOD/item.png" style="zoom:80%;" />

### 画布管理
图形绘制软件中经常需要根据当前绘制状态的不同对用户的输入产生不同的响应（例如绘制直线时鼠标点击与拖动图元时的鼠标点击），用一个状态标志来记录当前状态并对应不同状态调用不同的响应函数是一个合理的做法。故可用状态模式将不同状态下的响应函数进行抽象，这样可以避免巨大的条件逻辑块。同时可以在管理类中用一个`Map`存下状态与响应模块的映射，这样就可以采用注册的方式动态增添状态，增添新的状态不需要修改状态切换代码。具体的UML类图如下：
<img src="https://github.com/Hoyyyywolf/Images/raw/master/OOD/cc.png" style="zoom:80%;" />
...
## 界面展示
<img src="https://github.com/Hoyyyywolf/Images/raw/master/OOD/gui.png" style="zoom: 50%;" />